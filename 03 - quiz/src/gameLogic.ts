...
export async function askQuestions(questionsArray: Question[]): Promise<number> {
  ...
  for (const question of questionsArray) {
    ...
    if (question.correctAnswer === index + 1) {
      ...
      console.log(
        styleText('red', '✗ Wrong!') +
          ` The correct answer was: ${styleText(['green', 'underline'], question.answers[question.correctAnswer - 1])}\n`
      );
    }
  }
...
export function displayFinalResult(correctCount: number, totalQuestions: number): void {
  ...
  if (correctCount === totalQuestions) {
    console.log(
      styleText(
        'green',
        `🎉 Perfect! You got all ${correctCount} out of ${totalQuestions} correct!`
      )
    );
  } else if (correctCount >= totalQuestions / 2) {
    console.log(
      styleText('yellow', `👍 Good job! You got ${correctCount} out of ${totalQuestions} correct!`)
    );
  } else {
    console.log(
      styleText(
        'red',
        `📚 Keep learning! You got ${correctCount} out of ${totalQuestions} correct.`
      )
    );
  }
